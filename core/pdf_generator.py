...
class PDFGenerator:
    ...
    def generate(self, report_sections: Dict[str, Any], output_path: str) -> str:
        ...
        doc = SimpleDocTemplate(
            ...
            title="Buildway Tech 香港股票智能分析報告",
    ...
    def _toc_page(self, report_sections: dict) -> list:
        ...
        elements.append(Paragraph(
            "本報告由 Python 計算及多模組流程生成，所有財務數值由 Python 計算，LLM 只負責文字整理。",
            self.styles["SmallTC"],
        ))
        return elements
    ...
    def _cover(self, cover: Dict[str, Any]) -> List[Any]:
        ...
        elements.extend([
            ...
            Paragraph(cover.get("system", "香港股票智能分析系統"), self.styles["BodyTC"]),
            Spacer(1, 1.0 * cm),
        ...
        rows = [
            ...
            ["報告日期", cover.get("report_date", "N/A")],
        ]
        ...
        elements.append(Paragraph("本報告由 Python 計算及多模組流程生成；DeepSeek 只在啟用時協助文字整理，不參與數值計算。", self.styles["SmallTC"]))
        return elements

    def _market_snapshot(self, section: Dict[str, Any]) -> List[Any]:
        elements = [self._title("市場快照")]
        ...
        if price_sec:
            ...
            rows = [[k, v] for k, v in rows if _pdf_has_valid_value(v)]
            if rows:
    ...
    def _executive_summary(self, section: Dict[str, Any]) -> List[Any]:
        elements = [self._title("執行摘要")]
        confidence = _pdf_confidence_label(section.get("data_confidence_label"))
        metric_rows = [
            ["最終評級", section.get("final_rating", "N/A")],
            ["資料可信度", confidence],
            ["主要風險", section.get("key_risk", "N/A")],
            ["主要機會", section.get("key_opportunity", "N/A")],
            ["建議行動分類", section.get("recommended_action", "N/A")],
        ]
    ...
    def _multi_agent(self, section: Dict[str, Any]) -> List[Any]:
        elements = [self._title(section.get("title", "投資委員會討論摘要"))]
        rows = [["模組", "性格定位", "核心觀點", "正面因素", "主要憂慮", "信心分數", "對評級影響"]]
    ...
    def _financial(self, section: Dict[str, Any]) -> List[Any]:
        ...
        elements.append(Spacer(1, 0.2 * cm))
        elements.append(Paragraph("Python 計算財務指標", self.styles["SubTitle"]))
        rows = [["指標", "數值"]] + [[a, b] for a, b in _pdf_valid_rows(section.get("metrics", []) or [])]
        elements.append(self._table(rows, [6.0 * cm, 9.4 * cm], header=True))
        history = _pdf_valid_history(section.get("history", []) or [])
        if history:
            elements.extend([Spacer(1, 0.3 * cm), Paragraph("歷史財務", self.styles["SubTitle"])])
            elements.append(self._table([["年度", "收入", "EBITDA", "自由現金流"]] + history, [3.0 * cm, 4.1 * cm, 4.1 * cm, 4.2 * cm], header=True))
        return elements
    ...
    def _portfolio(self, section: Dict[str, Any]) -> List[Any]:
        elements = [self._title("組合與風險控制")]
        rows = [
            ["投資者適合度", section.get("investor_suitability", "")],
            ["倉位邏輯", section.get("position_sizing", "")],
            ["止蝕 / 檢視觸發", section.get("risk_control", "")],
            ["行動分類", section.get("action_category", "")],
        ]
    ...
    def _conclusion(self, section: Dict[str, Any], disclaimer: Dict[str, Any]) -> List[Any]:
        ...
        elements.extend([
            Spacer(1, 0.2 * cm),
            Paragraph("資料限制", self.styles["SubTitle"]),
        ...
        elements.extend([
            Spacer(1, 0.25 * cm),
            Paragraph(disclaimer.get("title", "免責聲明"), self.styles["SubTitle"]),
