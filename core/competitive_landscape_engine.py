...
def _build_peer_record(ticker: str, master: Dict[str, Any], report_data: Dict[str, Any]) -> Dict:
    ...
    # Try master data with multiple key formats:
    # "0728" -> try "0728", "0728.HK", "728.HK"
    t_hk = ticker if ticker.upper().endswith(".HK") else f"{ticker}.HK"
    stock_info = (
        master.get(ticker)
        or master.get(t_hk)
        or master.get(ticker.lstrip("0").zfill(4))
        or master.get(f"{ticker.lstrip('0').zfill(4)}.HK")
        or {}
    )
