"""Helper script to write fos_components.py"""
import os
...
def render_peer_comparison(report: Dict[str, Any]) -> None:
    ...
    for i, p in enumerate(peers):
        ...
        cols[i].markdown(
            ...
            f"<div style='display:grid;grid-template-columns:1fr 1fr;gap:0.4rem;margin-top:0.6rem;font-size:0.8rem'>"
            f"<div style='background:#fff;border-radius:6px;padding:0.3rem'><div style='color:#667085'>PE</div>"
            f"<div style='font-weight:600'>{_e(p.get('pe','N/A'))}</div></div>"
            f"<div style='background:#fff;border-radius:6px;padding:0.3rem'><div style='color:#667085'>PB</div>"
            f"<div style='font-weight:600'>{_e(p.get('pb','N/A'))}</div></div>"
            f"<div style='background:#fff;border-radius:6px;padding:0.3rem'><div style='color:#667085'>股息率</div>"
            f"<div style='font-weight:600'>{_e(p.get('div_yield','N/A'))}%</div></div>"
            f"<div style='background:#fff;border-radius:6px;padding:0.3rem'><div style='color:#667085'>市值</div>"
            f"<div style='font-weight:600;font-size:0.7rem'>{_e(p.get('mkt_cap','N/A'))}</div></div>"
            f"</div>"
            f"<div style='margin-top:0.6rem;background:{rc}22;border-radius:6px;padding:0.3rem;"
            f"color:{rc};font-weight:700'>風險 {risk}/10</div>"
            f"</div>",
            unsafe_allow_html=True,
        )
...
def render_investment_committee(report: Dict[str, Any]) -> None:
    ...
    summary = ic.get("committee_summary",
                     "投資委員會綜合各分析師觀點後，認為該股票具備中長線投資價值，但需留意短期波動風險。")
    verdict = ic.get("final_recommendation", "正面")
    vc = {"買入": "#15a36d", "正面": "#15a36d", "觀察": "#d9a441",
          "中性": "#667085", "減持": "#d64545", "避免": "#d64545"}.get(verdict, "#667085")
    ...
    with col_bull:
        st.markdown(
            "<div style='background:rgba(21,163,109,0.15);border:1px solid rgba(21,163,109,0.4);"
            "border-radius:10px;padding:1rem'>"
            "<div style='color:#4ade80;font-size:1.1rem;font-weight:700;margin-bottom:0.8rem'>🐂 牛市觀點</div>"
            "<div style='color:#86efac;font-size:0.75rem;margin-bottom:0.5rem'>市場分析 · 財務分析 · 新聞分析</div>",
            unsafe_allow_html=True,
        )
        for pt in bull_pts:
            st.markdown(f"<div style='color:#d1fae5;font-size:0.85rem;margin:0.3rem 0'>✅ {_e(pt)}</div>",
                        unsafe_allow_html=True)
        st.markdown(
    ...
    with col_bear:
        st.markdown(
            "<div style='background:rgba(214,69,69,0.15);border:1px solid rgba(214,69,69,0.4);"
            "border-radius:10px;padding:1rem'>"
            "<div style='color:#f87171;font-size:1.1rem;font-weight:700;margin-bottom:0.8rem'>🐻 熊市觀點</div>"
            "<div style='color:#fca5a5;font-size:0.75rem;margin-bottom:0.5rem'>風險分析 · 估值分析 · 事件分析</div>",
            unsafe_allow_html=True,
        )
        for pt in bear_pts:
            st.markdown(f"<div style='color:#fee2e2;font-size:0.85rem;margin:0.3rem 0'>⚠️ {_e(pt)}</div>",
                        unsafe_allow_html=True)
        st.markdown(
            f"<div style='margin-top:0.8rem;text-align:center;font-size:1.5rem;"
    ...
    st.markdown(
        f"<div style='background:rgba(217,164,65,0.1);border:1px solid rgba(217,164,65,0.3);"
...
def render_investment_conclusion(report: Dict[str, Any]) -> None:
    ...
    st.markdown(
        f"<div style='background:{cfg['bg']};border:2px solid {cfg['color']};"
        ...
        f"<div style='font-size:2.5rem;font-weight:800;color:{cfg['color']}'>{cfg['icon']} {rating}</div>"
        f"</div>"
        f"<div style='text-align:center'>"
        ...
        f"</div></div>"
        f"<div style='background:#fff;border-radius:8px;padding:1rem;font-size:0.9rem;color:#172033'>"
        f"{_e(summary)}</div></div>",
        unsafe_allow_html=True,
    )
...
dest = os.path.join(os.path.dirname(__file__), "core", "fos_components.py")
